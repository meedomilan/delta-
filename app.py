...
ENTRY_ZONE_BELOW_ATR = float(os.getenv("ENTRY_ZONE_BELOW_ATR", "0.08"))
ENTRY_ZONE_ABOVE_ATR = float(os.getenv("ENTRY_ZONE_ABOVE_ATR", "0.10"))
CONFIRM_VALID_MINUTES = int(os.getenv("CONFIRM_VALID_MINUTES", "12"))
...
app = FastAPI(title="Ahmed Retest Entry Trader", version="6.0.0")
...
def db() -> sqlite3.Connection:
    ...
    connection.execute("""
        CREATE TABLE IF NOT EXISTS after_stop_checkpoints (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            entry_key TEXT NOT NULL,
            minutes_after_sl INTEGER NOT NULL,
            price REAL NOT NULL,
            returned_to_entry INTEGER NOT NULL DEFAULT 0,
            tp1_after_sl INTEGER NOT NULL DEFAULT 0,
            tp2_after_sl INTEGER NOT NULL DEFAULT 0,
            tp3_after_sl INTEGER NOT NULL DEFAULT 0,
            continued_against INTEGER NOT NULL DEFAULT 0,
            favorable_after_sl_percent REAL NOT NULL DEFAULT 0,
            adverse_after_sl_percent REAL NOT NULL DEFAULT 0,
            evaluated_at INTEGER NOT NULL,
            UNIQUE(entry_key, minutes_after_sl)
        )
    """)
    ...
    ensure_column(connection, "setups", "confirmation_passed", "INTEGER NOT NULL DEFAULT 0")
    ensure_column(connection, "setups", "confirmation_time", "INTEGER")
    ensure_column(connection, "entries", "entry_mode", "TEXT")
    ensure_column(connection, "entries", "confirmation_time", "INTEGER")
...
async def get_latest_prices(
    ...
    """
    Binance قد يعيد استجابة فارغة/غير متوقعة مؤقتًا.
    لا نسمح لهذا بإيقاف البوت؛ نعيد قاموسًا فارغًا وتكمل الدورة التالية.
    """
    try:
        raw = await get_json(session, "/fapi/v1/ticker/price")
    except Exception as error:
        log.warning("Ticker price request failed: %s", error)
        return {}

    if not isinstance(raw, list):
        log.warning(
            "Ticker price returned unexpected payload: %s",
            type(raw).__name__,
        )
        return {}
...
def confirmed_entry_zone(setup: BreakoutSetup) -> Tuple[float, float]:
    """
    منطقة تنفيذ ضيقة حول مستوى الكسر.
    الهدف: بعد التأكيد لا نطارد السعر؛ ننتظر دخوله في منطقة شراء/بيع.
    """
    below = setup.atr_value * ENTRY_ZONE_BELOW_ATR
    above = setup.atr_value * ENTRY_ZONE_ABOVE_ATR
    return (
        setup.breakout_level - below,
        setup.breakout_level + above,
    )


def price_inside_confirmed_zone(
    setup: BreakoutSetup,
    price: float,
) -> bool:
    low, high = confirmed_entry_zone(setup)
    return low <= price <= high


def mark_confirmation(setup_key: str) -> int:
    now = int(time.time())
    with db() as connection:
        connection.execute("""
            UPDATE setups
            SET confirmation_passed = 1,
                confirmation_time = ?
            WHERE setup_key = ?
        """, (now, setup_key))
        connection.commit()
...
def clear_confirmation(setup_key: str) -> None:
    with db() as connection:
        connection.execute("""
            UPDATE setups
            SET confirmation_passed = 0,
...
def get_confirmation_state(setup_key: str) -> Tuple[bool, Optional[int]]:
    with db() as connection:
        row = connection.execute("""
            SELECT confirmation_passed, confirmation_time
            FROM setups
            WHERE setup_key = ?
        """, (setup_key,)).fetchone()
    ...
    return bool(row["confirmation_passed"]), (
        int(row["confirmation_time"])
        if row["confirmation_time"] else None
    )
...
def build_trade_plan(
    ...
    if setup.direction == "BUY":
        ...
        confirmed_low, confirmed_high = confirmed_entry_zone(setup)
        entry_low = confirmed_low
        entry_high = confirmed_high
...
def save_entry(
    ...
    with db() as connection:
        ...
        """, (
            ...
            int(time.time()),
            "CONFIRMED_ZONE",
            int(plan.get("confirmation_time") or time.time()),
        ))
        connection.commit()
...
def build_entry_message(
    ...
    title = (
        "🔵 دخول مؤكد من منطقة الشراء — شراء"
        if is_buy
        else
        "🔵 دخول مؤكد من منطقة البيع — بيع"
    )
    ...
    return (
        ...
        f"🧭 طريقة الدخول: <b>بعد التأكيد وداخل منطقة الشراء/البيع</b>\n"
        f"🚫 لا مطاردة للسعر خارج المنطقة\n"
...
async def retest_scan(
    ...
    # إذا Binance أعاد payload مؤقتًا غير صالح، لا نكسر الحلقة.
    if not price_map:
        STATE["last_retest_scan"] = datetime.now(TZ).isoformat()
        return
    ...
    for setup in setups:
        ...
        confirmed, confirmation_time = get_confirmation_state(setup.setup_key)

        # إذا التأكيد قديم، نلغيه وننتظر تأكيدًا جديدًا.
        if confirmed and confirmation_time:
            if int(time.time()) - confirmation_time > CONFIRM_VALID_MINUTES * 60:
                clear_confirmation(setup.setup_key)
                confirmed = False
        ...
        try:
            one_minute, five_minute = await asyncio.gather(
                ...
                get_micro_klines(session, setup.symbol, CONTEXT_TF, max(14, SWING_LOOKBACK_5M + 2)),
            )
            ...
            if not confirmed:
                ...
                confirmation_time = mark_confirmation(setup.setup_key)
                confirmed = True

                log.info(
                    "CONFIRMED %s %s %s; waiting entry zone",
                    setup.symbol,
                    setup.timeframe,
                    setup.direction,
                )

            # بعد التأكيد: لا ندخل إذا السعر ابتعد، بل ننتظر منطقة الشراء/البيع.
            if not price_inside_confirmed_zone(setup, price):
                continue

            # حماية إضافية من chasing
            ...
            plan["confirmation_time"] = confirmation_time
            ...
            log.info(
                "ENTRY SENT CONFIRMED_ZONE %s %s %s price=%s",
                setup.symbol,
                setup.timeframe,
                setup.direction,
                price,
            ...
            log.warning(
                "Retest confirm/entry failed %s: %s",
                setup.setup_key,
                error,
            )
...
AFTER_STOP_CHECKPOINTS = (5, 15, 30, 60, 120)
...
async def evaluate_after_stop(session: aiohttp.ClientSession) -> None:
    now = int(time.time())
    ...
    with db() as connection:
        entries = connection.execute("""
            SELECT *
            FROM entries
            WHERE first_event = 'SL_FIRST'
              AND sl_hit_time IS NOT NULL
              AND created_at >= ?
            ORDER BY sl_hit_time ASC
            LIMIT 1000
        """, (now - 14 * 86400,)).fetchall()

    for entry in entries:
        sl_time = int(entry["sl_hit_time"])
        entry_price = float(entry["entry_price"])
        stop_loss = float(entry["stop_loss"])
        direction = entry["direction"]

        for minutes_after in AFTER_STOP_CHECKPOINTS:
            due = sl_time + minutes_after * 60
            if now < due:
                continue

            with db() as connection:
                exists = connection.execute("""
                    SELECT id FROM after_stop_checkpoints
                    WHERE entry_key = ? AND minutes_after_sl = ?
                """, (entry["entry_key"], minutes_after)).fetchone()
            ...
            try:
                ...
                if direction == "BUY":
                    returned_to_entry = int(data["high"].max() >= entry_price)
                    tp1_after = int(data["high"].max() >= float(entry["tp1"]))
                    tp2_after = int(data["high"].max() >= float(entry["tp2"]))
                    tp3_after = int(data["high"].max() >= float(entry["tp3"]))
                    favorable = (data["high"].max() - stop_loss) / stop_loss * 100
                    adverse = (data["low"].min() - stop_loss) / stop_loss * 100
                    continued_against = int(final_price < stop_loss)
                else:
                    returned_to_entry = int(data["low"].min() <= entry_price)
                    tp1_after = int(data["low"].min() <= float(entry["tp1"]))
                    tp2_after = int(data["low"].min() <= float(entry["tp2"]))
                    tp3_after = int(data["low"].min() <= float(entry["tp3"]))
                    favorable = (stop_loss - data["low"].min()) / stop_loss * 100
                    adverse = (stop_loss - data["high"].max()) / stop_loss * 100
                    continued_against = int(final_price > stop_loss)

                with db() as connection:
                    connection.execute("""
                        INSERT OR IGNORE INTO after_stop_checkpoints (
                            entry_key, minutes_after_sl, price,
                            returned_to_entry,
                            tp1_after_sl, tp2_after_sl, tp3_after_sl,
                            continued_against,
                            favorable_after_sl_percent,
                            adverse_after_sl_percent,
                            evaluated_at
                        )
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (
                        ...
                        tp3_after,
                        continued_against,
                        round(float(favorable), 4),
                        round(float(adverse), 4),
                        now,
                    ))
                    connection.commit()
                ...
                log.warning(
                    "After-stop checkpoint error %s %sm: %s",
                    entry["entry_key"],
                    minutes_after,
                    error,
                )
        ...
        while True:
            ...
            try:
                ...
                if (
                    ...
                    await evaluate_after_stop(session)
...
@app.get("/stats/success")
async def stats_success() -> Dict[str, Any]:
    with db() as connection:
        rows = connection.execute("""
            SELECT
                direction,
                ...
                SUM(CASE WHEN first_event = 'TP1_FIRST' THEN 1 ELSE 0 END) AS tp1_first,
                SUM(CASE WHEN first_event = 'SL_FIRST' THEN 1 ELSE 0 END) AS sl_first,
                SUM(CASE WHEN first_event IS NULL THEN 1 ELSE 0 END) AS pending,
                ROUND(AVG(max_mfe_percent), 4) AS avg_mfe,
                ROUND(AVG(max_mae_percent), 4) AS avg_mae
            FROM entries
            GROUP BY direction
    ...
    for row in rows:
        ...
        item["tp1_before_sl_rate"] = round((item["tp1_first"] or 0) / decided * 100, 2) if decided else None
        item["tp1_hit_rate"] = round((item["tp1_hits"] or 0) / entries * 100, 2) if entries else None
        item["tp2_hit_rate"] = round((item["tp2_hits"] or 0) / entries * 100, 2) if entries else None
        item["tp3_hit_rate"] = round((item["tp3_hits"] or 0) / entries * 100, 2) if entries else None
        table.append(item)

    return {"title": "Trade Success Statistics", "table": table}


@app.get("/stats/after-stop")
async def stats_after_stop() -> Dict[str, Any]:
    with db() as connection:
        rows = connection.execute("""
            SELECT
                minutes_after_sl,
                COUNT(*) AS cases,
                SUM(returned_to_entry) AS returned_to_entry,
                SUM(tp1_after_sl) AS tp1_after_sl,
                SUM(tp2_after_sl) AS tp2_after_sl,
                SUM(tp3_after_sl) AS tp3_after_sl,
                SUM(continued_against) AS continued_against,
                ROUND(AVG(favorable_after_sl_percent), 4) AS avg_favorable_after_sl,
                ROUND(AVG(adverse_after_sl_percent), 4) AS avg_adverse_after_sl
            FROM after_stop_checkpoints
            GROUP BY minutes_after_sl
            ORDER BY minutes_after_sl
        """).fetchall()
    ...
    for row in rows:
        ...
        for key in ["returned_to_entry", "tp1_after_sl", "tp2_after_sl", "tp3_after_sl", "continued_against"]:
            item[f"{key}_rate"] = round((item[key] or 0) / cases * 100, 2) if cases else None
        table.append(item)

    return {"title": "After Stop-Loss Statistics", "table": table}


@app.get("/stats/after-stop/by-direction")
async def stats_after_stop_by_direction() -> Dict[str, Any]:
    with db() as connection:
        rows = connection.execute("""
            SELECT
                entries.direction,
                after_stop_checkpoints.minutes_after_sl,
                COUNT(*) AS cases,
                SUM(after_stop_checkpoints.returned_to_entry) AS returned_to_entry,
                SUM(after_stop_checkpoints.tp1_after_sl) AS tp1_after_sl,
                SUM(after_stop_checkpoints.tp2_after_sl) AS tp2_after_sl,
                SUM(after_stop_checkpoints.tp3_after_sl) AS tp3_after_sl,
                SUM(after_stop_checkpoints.continued_against) AS continued_against,
                ROUND(AVG(after_stop_checkpoints.favorable_after_sl_percent), 4) AS avg_favorable_after_sl,
                ROUND(AVG(after_stop_checkpoints.adverse_after_sl_percent), 4) AS avg_adverse_after_sl
            FROM after_stop_checkpoints
            INNER JOIN entries
                ON entries.entry_key = after_stop_checkpoints.entry_key
            GROUP BY entries.direction, after_stop_checkpoints.minutes_after_sl
            ORDER BY entries.direction, after_stop_checkpoints.minutes_after_sl
        """).fetchall()
    ...
    return {"title": "After Stop-Loss By Direction", "table": table}
...
async def dashboard() -> HTMLResponse:
    with db() as connection:
        ...
        tp2 = connection.execute(
            "SELECT COUNT(*) AS c FROM entries WHERE tp2_hit=1"
        ).fetchone()["c"]
        tp3 = connection.execute(
            "SELECT COUNT(*) AS c FROM entries WHERE tp3_hit=1"
        ).fetchone()["c"]
        ...
        direction_rows = connection.execute("""
            SELECT
                ...
                SUM(CASE WHEN tp3_hit=1 THEN 1 ELSE 0 END) AS tp3,
                SUM(CASE WHEN sl_hit=1 THEN 1 ELSE 0 END) AS sl,
                SUM(CASE WHEN first_event='TP1_FIRST' THEN 1 ELSE 0 END) AS tp1_first,
                SUM(CASE WHEN first_event='SL_FIRST' THEN 1 ELSE 0 END) AS sl_first,
                SUM(CASE WHEN first_event IS NULL THEN 1 ELSE 0 END) AS pending,
                ROUND(AVG(max_mfe_percent), 3) AS avg_mfe,
                ROUND(AVG(max_mae_percent), 3) AS avg_mae
            FROM entries
            GROUP BY direction
            ORDER BY direction
        """).fetchall()
        ...
        checkpoint_rows = connection.execute("""
            SELECT
                ...
                checkpoints.minutes_after,
                COUNT(*) AS cases,
                ROUND(AVG(checkpoints.return_percent), 3) AS avg_return,
                ROUND(AVG(checkpoints.mfe_percent), 3) AS avg_mfe,
                ROUND(AVG(checkpoints.mae_percent), 3) AS avg_mae
            FROM checkpoints
            INNER JOIN entries ON entries.entry_key=checkpoints.entry_key
            GROUP BY entries.direction, checkpoints.minutes_after
            ORDER BY entries.direction, checkpoints.minutes_after
        """).fetchall()
        ...
        after_rows = connection.execute("""
            SELECT
                minutes_after_sl,
                COUNT(*) AS cases,
                SUM(returned_to_entry) AS returned,
                SUM(tp1_after_sl) AS tp1_after,
                SUM(tp2_after_sl) AS tp2_after,
                SUM(tp3_after_sl) AS tp3_after,
                SUM(continued_against) AS against,
                ROUND(AVG(favorable_after_sl_percent), 3) AS avg_fav,
                ROUND(AVG(adverse_after_sl_percent), 3) AS avg_adv
            FROM after_stop_checkpoints
            GROUP BY minutes_after_sl
            ORDER BY minutes_after_sl
        """).fetchall()
        ...
        recent = connection.execute("""
            SELECT
                ...
                tp1, tp2, tp3, first_event,
                max_mfe_percent, max_mae_percent, created_at,
                entry_mode
            FROM entries
            ORDER BY created_at DESC
            LIMIT 50
    ...
    decided = tp1_first + sl_first
    success_rate = pct(tp1_first, decided)
    ...
    for row in direction_rows:
        decided_dir = (row["tp1_first"] or 0) + (row["sl_first"] or 0)
        direction_html += tr([
            "🟢 BUY" if row["direction"] == "BUY" else "🔴 SELL",
            row["entries"],
            f"{pct(row['tp1_first'] or 0, decided_dir):.1f}%",
            row["tp1"] or 0,
            row["tp2"] or 0,
            row["tp3"] or 0,
            row["sl"] or 0,
            row["pending"] or 0,
            f"{row['avg_mfe'] or 0:.3f}%",
            f"{row['avg_mae'] or 0:.3f}%",
        ])

    checkpoints_html = "".join(
        tr([
            "🟢 BUY" if row["direction"] == "BUY" else "🔴 SELL",
            f"{row['minutes_after']} د",
            row["cases"],
            f"{row['avg_return'] or 0:.3f}%",
            f"{row['avg_mfe'] or 0:.3f}%",
            f"{row['avg_mae'] or 0:.3f}%",
        ])
    ...
    for row in after_rows:
        ...
        after_html += tr([
            f"{row['minutes_after_sl']} د",
            cases,
            f"{pct(row['returned'] or 0, cases):.1f}%",
            f"{pct(row['tp1_after'] or 0, cases):.1f}%",
            f"{pct(row['tp2_after'] or 0, cases):.1f}%",
            f"{pct(row['tp3_after'] or 0, cases):.1f}%",
            f"{pct(row['against'] or 0, cases):.1f}%",
            f"{row['avg_fav'] or 0:.3f}%",
            f"{row['avg_adv'] or 0:.3f}%",
        ])

    recent_html = ""
    for row in recent:
        ts = datetime.fromtimestamp(row["created_at"], TZ).strftime("%d-%m %H:%M")
        recent_html += tr([
            f"#{row['symbol']}.P",
            "🟢 BUY" if row["direction"] == "BUY" else "🔴 SELL",
            f"{row['entry_price']:.10g}",
            f"{row['stop_loss']:.10g}",
            f"{row['tp1']:.10g}",
            f"{row['tp2']:.10g}",
            f"{row['tp3']:.10g}",
            row["first_event"] or "قيد المتابعة",
            f"{row['max_mfe_percent'] or 0:.2f}%",
            f"{row['max_mae_percent'] or 0:.2f}%",
            row["entry_mode"] or "CONFIRMED_ZONE",
            ts,
        ])
...
<title>Ahmed Retest Entry Trader — Statistics</title>
<style>
:root {{
  --bg:#0b1117; --card:#111a24; --card2:#172334; --line:#26384b;
  --text:#f6f8fb; --muted:#91b8e8; --good:#52d273; --bad:#ff6b6b; --warn:#ffc857;
}}
*{{box-sizing:border-box}}
body{{margin:0;background:var(--bg);color:var(--text);font-family:Arial,Tahoma,sans-serif}}
...
.card{{background:linear-gradient(180deg,var(--card2),var(--card));border:1px solid var(--line);border-radius:16px;padding:20px;text-align:center}}
.card .n{{font-size:34px;font-weight:800}}
.card .l{{color:var(--muted);font-size:16px;margin-top:6px}}
section{{margin:30px 0}}
h2{{font-size:25px;margin:0 0 13px}}
.tablebox{{overflow:auto;border:1px solid var(--line);border-radius:14px}}
table{{width:100%;border-collapse:collapse;min-width:950px;background:var(--card)}}
th{{background:#182537;color:#fff;padding:14px 10px;border-bottom:1px solid var(--line);white-space:nowrap}}
td{{padding:13px 10px;border-bottom:1px solid #203040;text-align:center;white-space:nowrap}}
tr:hover td{{background:#14202c}}
.good{{color:var(--good)}} .bad{{color:var(--bad)}} .warn{{color:var(--warn)}}
.links{{display:flex;gap:10px;flex-wrap:wrap;margin:16px 0}}
.links a{{color:#b9d8ff;text-decoration:none;background:#152234;border:1px solid var(--line);padding:8px 12px;border-radius:10px}}
.note{{font-size:13px;color:#91a1b5;margin-top:20px}}
</style>
</head>
<body>
<div class="wrap">
  <h1>📊 Ahmed Retest Entry Trader</h1>
  <div class="sub">1H/4H اتجاه → 15M كسر → 5M إعادة اختبار → 1M تأكيد → دخول من المنطقة فقط</div>

  <div class="links">
    <a href="/stats">JSON Stats</a>
    <a href="/stats/success">Success</a>
    <a href="/stats/after-stop">After Stop</a>
    <a href="/trades">Trades</a>
    <a href="/active-setups">Active Setups</a>
    <a href="/health">Health</a>
  </div>

  <div class="cards">
    <div class="card"><div class="n">{total_setups}</div><div class="l">كل الكسور</div></div>
    <div class="card"><div class="n">{touched}</div><div class="l">إعادة اختبار</div></div>
    <div class="card"><div class="n">{total_entries}</div><div class="l">دخول مؤكد</div></div>
    <div class="card"><div class="n">{success_rate:.1f}%</div><div class="l">TP1 قبل SL</div></div>
    <div class="card"><div class="n">{tp1}</div><div class="l">وصلت TP1</div></div>
    <div class="card"><div class="n">{tp2}</div><div class="l">وصلت TP2</div></div>
    <div class="card"><div class="n">{tp3}</div><div class="l">وصلت TP3</div></div>
    <div class="card"><div class="n">{pending}</div><div class="l">قيد المتابعة</div></div>
  </div>

  <section>
    <h2>🏆 نجاح الصفقات حسب الاتجاه</h2>
    <div class="tablebox"><table>
      <thead><tr>
        <th>الاتجاه</th><th>الصفقات</th><th>TP1 قبل SL</th>
        <th>TP1</th><th>TP2</th><th>TP3</th><th>SL</th><th>معلقة</th>
        <th>Avg MFE</th><th>Avg MAE</th>
      </tr></thead>
      <tbody>{direction_html}</tbody>
    </table></div>
  </section>

  <section>
    <h2>⏱ أداء الدخول بعد 5 / 15 / 30 / 60 دقيقة</h2>
    <div class="tablebox"><table>
      <thead><tr>
        <th>الاتجاه</th><th>المدة</th><th>الحالات</th>
        <th>متوسط العائد</th><th>متوسط MFE</th><th>متوسط MAE</th>
      </tr></thead>
      <tbody>{checkpoints_html}</tbody>
    </table></div>
  </section>

  <section>
    <h2>🛑 ماذا حدث بعد ضرب الوقف؟</h2>
    <div class="tablebox"><table>
      <thead><tr>
        <th>بعد SL</th><th>الحالات</th><th>رجع للدخول</th>
        <th>وصل TP1 لاحقًا</th><th>TP2</th><th>TP3</th>
        <th>استمر ضدنا</th><th>أفضل حركة بعد SL</th><th>أسوأ حركة بعد SL</th>
      </tr></thead>
      <tbody>{after_html}</tbody>
    </table></div>
  </section>

  <section>
    <h2>📋 آخر 50 صفقة بالتفصيل</h2>
    <div class="tablebox"><table>
      <thead><tr>
        <th>العملة</th><th>الاتجاه</th><th>الدخول</th><th>SL</th>
        <th>TP1</th><th>TP2</th><th>TP3</th><th>النتيجة الأولى</th>
        <th>MFE</th><th>MAE</th><th>طريقة الدخول</th><th>الوقت</th>
      </tr></thead>
      <tbody>{recent_html}</tbody>
    </table></div>
  </section>

  <div class="note">تتحدث الصفحة تلقائيًا كل 60 ثانية. النسب إحصائية من البيانات المسجلة وليست ضمانًا للنتائج المستقبلية.</div>
</div>
</body>
</html>
"""
