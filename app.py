...
SL_BUFFER_ATR = float(os.getenv("SL_BUFFER_ATR", "0.16"))
...
app = FastAPI(title="Ahmed Retest Entry Trader", version="1.0.0")
...
def build_trade_plan(
    ...
    closed = confirmation_data.iloc[:-1].tail(4)
    zone_low, zone_high = retest_zone(setup)
    ...
    if setup.direction == "BUY":
        ...
        max_risk = setup.atr_value * MAX_RISK_ATR
        risk = min(max(raw_risk, min_risk), max_risk)
...
async def retest_scan(
    ...
    for setup in setups:
        ...
        try:
            ...
            plan = build_trade_plan(setup, one_minute, price)
        ...
        try:
            ...
            for _, candle in data.iterrows():
                if entry["direction"] == "BUY":
                    sl_hit = candle["low"] <= float(entry["stop_loss"])
                    tp1_hit = candle["high"] >= float(entry["tp1"])
                else:
                    sl_hit = candle["high"] >= float(entry["stop_loss"])
                    tp1_hit = candle["low"] <= float(entry["tp1"])

                if sl_hit and tp1_hit:
                    outcome = "AMBIGUOUS_SAME_1M"
                    break
...
async def stats() -> Dict[str, Any]:
    with db() as connection:
        ...
        tp1_first = connection.execute(
            "SELECT COUNT(*) AS c FROM entries WHERE outcome = 'TP1_FIRST'"
        ).fetchone()["c"]

        sl_first = connection.execute(
            "SELECT COUNT(*) AS c FROM entries WHERE outcome = 'SL_FIRST'"
        ).fetchone()["c"]

        ambiguous = connection.execute(
            "SELECT COUNT(*) AS c FROM entries WHERE outcome = 'AMBIGUOUS_SAME_1M'"
        ).fetchone()["c"]
